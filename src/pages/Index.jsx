import Navbar from "@/components/Navbar";
import ZevHero from "@/components/zev/ZevHero";
import TrustStrip from "@/components/zev/TrustStrip";
import { MaturityLegend } from "@/components/zev/MaturityBadge";
import IndexBelowFold from "./IndexBelowFold";

const Index = () => {
  return (
    <div className="min-h-screen bg-background">
      <Navbar />
      {/* The trust strip and the label legend ride inside the hero's dark zone,
          so the graphite run continues straight into the key animation instead
          of sandwiching a sliver of light page between two dark zones. */}
      <ZevHero>
        <TrustStrip />
        <div className="container mx-auto px-4 pb-16 lg:px-8">
          <MaturityLegend />
        </div>
      </ZevHero>
      <IndexBelowFold />
    </div>
  );
};

export default Index;
